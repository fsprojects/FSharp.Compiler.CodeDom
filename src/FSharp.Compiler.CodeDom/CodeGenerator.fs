module FSharp.Compiler.CodeDom.Internal.Generator
...
open FSharp.Compiler.CodeDom.Internal.Visitor
...
let createContext (wr:TextWriter) (opts:CodeGeneratorOptions) (addopts) =
  { UniqueID = (Guid.NewGuid()).ToString("N")
    Writer = new IndentedTextWriter(wr);
    TypeRenames = Map.empty
    CurrentType = null;
    CurrentNamespace = ""
    DeclaredEvents = []
    BaseTypes = (None, [])
...
let (-!) (ctx:Context -> Context) (str:String) x =
  let c = (ctx x)
  c.Writer.Write(makeEscapedIdentifier str)
  c
